...
import { getConstrainedWidth, getConstrainedHeight, calculateResizeResult } from '../utils/constraints'
...
export function findTopNode(x: number, y: number, nodes: CanvasNode[]): CanvasNode | null {
  return nodes.find((n) => inBounds(x, y, { x: n.x, y: n.y, width: getConstrainedWidth(n), height: getConstrainedHeight(n) })) ?? null
}

export function findHitAnchor(x: number, y: number, node: CanvasNode): CanvasResizeAnchor | null {
  ...
  const hitBoxes: Record<CanvasResizeAnchor, HitBox> = {
    'top-left':     { x: node.x - aw / 2,       y: node.y - aw / 2,        width: cornerSize, height: cornerSize },
    'top-middle':   { x: node.x + (w - ah) / 2,  y: node.y - aw / 2,        width: ah,         height: aw         },
    'top-right':    { x: node.x + w - ah,         y: node.y - aw / 2,        width: cornerSize, height: cornerSize },
    'right-middle': { x: node.x + w - aw / 2,     y: node.y + (h - ah) / 2,  width: aw,         height: ah         },
    'bottom-right': { x: node.x + w - ah,         y: node.y + h - ah,        width: cornerSize, height: cornerSize },
    'bottom-middle':{ x: node.x + (w - ah) / 2,  y: node.y + h - aw / 2,    width: ah,         height: aw         },
    'bottom-left':  { x: node.x - aw / 2,        y: node.y + h - ah,        width: cornerSize, height: cornerSize },
    'left-middle':  { x: node.x - aw / 2,        y: node.y + (h - ah) / 2,  width: aw,         height: ah         },
  }

  for (const [anchor, box] of Object.entries(hitBoxes) as [CanvasResizeAnchor, HitBox][]) {
...
export function applyAnchorResize(
  ...
  const resizeAxes: Record<CanvasResizeAnchor, Array<'left' | 'right' | 'top' | 'bottom'>> = {
    'top-left':     ['left', 'top'],
    'top-middle':   ['top'],
    'top-right':    ['right', 'top'],
    'right-middle': ['right'],
  ...
  for (const edge of resizeAxes[anchor]) {
    ...
    const result = calculateResizeResult(edge, isHorizontal ? mouseX : mouseY, node)
