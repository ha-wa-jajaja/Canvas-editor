...
async function readJson(page: Page): Promise<unknown[]> {
  const text = await page.locator('.te-demo__output pre').textContent({ timeout: 5000 })
...
async function waitForJsonLength(page: Page, expected: number, timeout = 8000) {
  await page.waitForFunction(
    (len: number) => {
      ...
      try { return JSON.parse(pre?.textContent ?? '[]').length === len } catch { return false }
    },
    expected,
    { timeout },
  )
...
describe('Template Editor – browser integration', () => {
  ...
  it('loads blank canvas (0 items)', async () => {
    const page = await openPage(browser)
    await page.selectOption('.te-demo__control-group:last-of-type .te-demo__select', { label: '— blank canvas —' })
    await page.click('button.te-demo__btn')
  ...
  it('dragging a node changes its coordinates in the JSON output', async () => {
    ...
    const before = await readJson(page) as Array<{ x_min: number; y_min: number }>
    ...
    const after = await readJson(page) as Array<{ x_min: number; y_min: number }>
    const shifted = before.some((b, i) => {
      const a = after[i]
      return a && (Math.abs(a.x_min - b.x_min) > 0.5 || Math.abs(a.y_min - b.y_min) > 0.5)
    })
  ...
  it('resizing via an anchor updates item dimensions in the JSON output', async () => {
    ...
    const before = await readJson(page) as Array<{ x_max: number; y_max: number }>
    ...
    const after = await readJson(page) as Array<{ x_max: number; y_max: number }>
    const resized = before.some((b, i) => {
      const a = after[i]
      return a && (Math.abs(a.x_max - b.x_max) > 0.5 || Math.abs(a.y_max - b.y_max) > 0.5)
    })
  ...
  it('changing canvas format rescales the canvas height proportionally', async () => {
    ...
    const heightBefore = (await page.locator('canvas').first().boundingBox())!.height
    ...
    const heightAfter = (await page.locator('canvas').first().boundingBox())!.height
